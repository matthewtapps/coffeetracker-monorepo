import { Interval, addDays, eachDayOfInterval, endOfDay, format, isEqual } from "date-fns";
...
export const Charts = ({ data }: ChartsProps) => {
    ...
    const [date, setDate] = React.useState<DateRange | undefined>({ from: DEFAULT_DATE_RANGE.start, to: DEFAULT_DATE_RANGE.end })
    ...
    return (
        <div>
            <DatePickerWithRange date={date} setDate={handleDateChange} className="m-4" dataDateRange={getDataDateRangeWithStartPadding(data)} />
            <Card className="m-4">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="mx-auto text-inherit">Number of Shots</CardTitle>
                </CardHeader>
                <CardContent className="m-0 p-0">
                    <ResponsiveContainer width="100%" aspect={1.77} className="pr-8 pb-2">
                        <LineChart width={400} height={400} data={getDateNormalisedChartData(data, getInterval(date))}>
                            <CartesianGrid strokeDasharray="4 4" />
                            <XAxis dataKey="date" tickFormatter={dateFormatter} type="category"/>
                            <YAxis dataKey="cumulativeCount" type="number" />
                            <Tooltip wrapperStyle={{ background: "inherit" }} contentStyle={{ background: "inherit" }} labelFormatter={dateFormatter}/>
                            <Line type="monotone" dataKey="count" stroke={theme.theme === "dark" ? "white" : "black"} name="Count"/>
                            <Line type="monotone" dataKey="cumulativeCount" stroke={theme.theme === "dark" ? "white" : "black"} name="Cumulative Count" />
                        </LineChart>
                    </ResponsiveContainer>
                </CardContent>
            </Card>
...
const getInterval = (date: DateRange | undefined): Interval => {
    ...
    return { start: date.from || getDefaultStartDate(), end: date.to || getDefaultEndDate() }
}
...
const getDateNormalisedChartData = (date: Coffee[], interval: Interval): { date: Date, count: number, cumulativeCount: number }[] => {
    /**
...
const getDataDateRangeWithStartPadding = (date: Coffee[]): { from: Date, to: Date } => {
    /**
    ...
    return {
        from: addDays(dates.reduce((a, b) => a < b ? a : b), -1),
